...
def google_news_search(keywords, nb_url):
    """
    ...
    for keyword in keywords:
        query += keyword + ' '
    sites = []
    for url in search(query, tld='com', lang='en', num=nb_url, stop=nb_url, pause=3, tpe='nws'):
        #print(url)
        sites.append(url)
    return(sites)

def google_news_search2(keywords, nb_url):
