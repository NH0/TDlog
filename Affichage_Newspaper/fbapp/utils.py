...
def find_article_by_keywords(keywords): #prend en argument une liste de string contenant les mots clés
    articlesMatched = []
    for key in keywords:
        article_c = Article_c.query.filter(Article_c.keywords.lower() == keyword).all() # Il faudrait un autre moyen de rendre les keywords des articles insensibles à la casse, dès l'enregistrement dans la BdD par exemple
        if(len(article_c)>0):
